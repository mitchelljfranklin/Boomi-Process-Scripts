...
/*Notes*/
//Create a Retry Count Dynamic process Property and set to 0 at the start of the flow via Set Properties Shape; this is used to track the retry count
//Create a Retry Dynamic process Property and set to a static at the start of the flow via Set Properties Shape; this is used to track if retry is to continue or not.
...
if(retryCount <= maxRetry){
     ExecutionUtil.setDynamicProcessProperty("<$Retry Dynamic Process Property Name$>", 'true', true); //Retry still required
        int waitFor = retryWait[retryCount];
        Thread.sleep(waitFor * 1000);

} else {
     ExecutionUtil.setDynamicProcessProperty("<$Retry Dynamic Process Property Name$>", 'false', true); //Retry no longer required
}
